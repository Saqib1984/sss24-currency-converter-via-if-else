...
while (condition) {
    ...
    if (from && to && amount) {
        ...
        console.log(chalk.magenta.bold(`Your Converted Amount ${from} to ${to} is ${result.toFixed(2)}.${answer.to}`));
    }
    else {
        console.log(chalk.bgRed("You Timed Out, Please Try Again Letter"));
    }
    console.log(chalk.yellow.bold("\t THANKS YOU FOR USING SSS MONEY EXCHANGE\t"));
    let answer_2 = await inquirer.prompt({
        name: "exit",
        type: "confirm",
        message: chalk.bgRed(`If You Want To Perform Another Transaction, Please Enter "Y", For Exit Enter "N"`),
        default: false,
    });
